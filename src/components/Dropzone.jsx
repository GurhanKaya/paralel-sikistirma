    ...
    <div className="w-full">
      <div
        ...
        className={`
          ...
          py-16 px-8 text-center transition-all duration-300
          ${disabled ? "opacity-50 cursor-not-allowed" : ""}
          ${dragging
            ? "border-violet-400 bg-violet-500/10 scale-[1.01]"
            : file
            ? "border-emerald-500/60 bg-emerald-500/5"
            : "border-white/15 hover:border-violet-400/50 bg-white/[0.02] hover:bg-violet-500/5"}
        `}
      >
        <input
          ref={inputRef}
        ...
        {file ? (
          <div className="space-y-2">
            ...
            <p className="text-slate-100 font-semibold text-base break-all px-4">{file.name}</p>
            <p className="text-slate-400 text-sm">{formatBytes(file.size)}</p>
            <p className="text-violet-400 text-xs font-medium mt-1">Degistirmek icin tikla</p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <div>
              <p className="text-slate-200 font-semibold text-base">Dosyayı buraya bırak</p>
